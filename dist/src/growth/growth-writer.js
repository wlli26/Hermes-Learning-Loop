        ...
        try {
            ...
            if (dedupeSet.has(normalizedSkill.slug)) {
                ...
                if (existing) {
                    ...
                    console.warn(`[GrowthWriter] Skill ${normalizedSkill.slug} in dedupeHints but not in state, recreating`);
            ...
            console.error(`[GrowthWriter] Failed to save skill ${normalizedSkill.slug}:`, error instanceof Error ? error.message : String(error));
            // 清理失败的目录
            try {
                ...
                console.error(`[GrowthWriter] Failed to cleanup directory ${skillDir}:`, cleanupError instanceof Error ? cleanupError.message : String(cleanupError));
            }
        }
    }
}
...
function ensureUseWhenSentence(value) {
...
function readConfidence(value, fallback) {
