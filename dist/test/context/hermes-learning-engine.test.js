...
describe("createHermesLearningEngine", () => {
    ...
    it("skips recursive review runs for review worker sessions", async () => {
        ...
        await engine.afterTurn?.({
            ...
            messages: [{ role: "user", content: "这里发生过一次重试、一次改道、一次纠正。" }],
            prePromptMessageCount: 0,
        });
    ...
    it("filters bootstrap noise out of the review summary and keeps the substantive task", async () => {
        ...
        await engine.afterTurn?.({
            ...
            messages: [
                ...
                },
                {
                    role: "toolResult",
                    name: "read",
                    content: "{\"status\":\"error\",\"tool\":\"read\",\"error\":\"ENOENT: no such file or directory, access '/root/.openclaw/workspace/memory/2026-04-12.md'\"}",
                },
                {
                    role: "user",
        ...
        expect(buildReviewPrompt).toHaveBeenCalledWith(expect.objectContaining({
