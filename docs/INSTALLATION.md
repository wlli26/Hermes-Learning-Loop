...
本文档说明如何在 OpenClaw 中安装和启用 Hermes Learning 插件。

## 方式一：压缩包下载（推荐，无需 Git）

适用于无法使用 Git 或需要完全离线安装的场景。
...
```bash
# 下载 tar.gz
curl -L https://git.iflytek.com/hy_spark_agent_builder/workflow-skill/claw-learning-loop/-/archive/develop/claw-learning-loop-develop.tar.gz | tar -xz
cd claw-learning-loop-develop

# 或下载 zip
curl -L -o claw-learning-loop.zip https://git.iflytek.com/hy_spark_agent_builder/workflow-skill/claw-learning-loop/-/archive/develop/claw-learning-loop-develop.zip
unzip claw-learning-loop.zip
cd claw-learning-loop-develop
```
...
```bash
curl -L https://github.com/wlli26/Hermes-Learning-Loop/archive/refs/heads/main.tar.gz | tar -xz
cd Hermes-Learning-Loop-main
```
...
`install.sh` 会按优先级选择执行方式：

1. **优先用 Node.js**：调用 `node bin/cli.js install`（如果系统有 Node.js）
2. **降级用 Python**：直接操作 JSON 配置文件（Mac/Linux 通常自带 Python）
3. **两者都没有**：打印手动配置说明

安装操作：
- 把插件入口（`dist/src/index.js`）写入 `~/.openclaw/openclaw.json` 的 `plugins.load.paths`
- 把 `plugins.slots.contextEngine` 设置为 `hermes-learning`
- 在 `plugins.entries.hermes-learning.enabled` 中显式启用
- 写入 `plugins.installs.index` 安装记录
- 自动备份原配置文件（`openclaw.json.<timestamp>.bak`）

### 兼容性

- **Mac/Linux**：直接运行 `.sh` 脚本
- **Windows**：需要 Git Bash 或 WSL
- **离线安装**：完全支持，`dist/` 已随包提交，无需 `npm install`
...
```bash
# 克隆仓库
git clone https://git.iflytek.com/hy_spark_agent_builder/workflow-skill/claw-learning-loop.git -b develop
cd claw-learning-loop
