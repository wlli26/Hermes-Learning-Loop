export function buildReviewPrompt(params: {
  ...
  existingSkills?: Array<{ slug: string; summary: string }>;
}) {
  const sections = [
  ...
  // 添加现有知识上下文
  if (
    (params.existingMemories && params.existingMemories.length > 0) ||
    ...
    sections.push(
      ...
      "",
      "If a memory candidate matches existing knowledge, include its title in dedupeHints and skip memoryCandidates.",
      "If a skill should be updated, include its slug in BOTH dedupeHints AND skillCandidates (with the full revised SKILL.md).",
      "Check existing skill slugs carefully — prefer reusing a close match over creating a new slug.",
    );
