...
describe("applyGrowthResult", () => {
  ...
  it("filters out low-confidence candidates below threshold", () => {
    ...
    const memoryContent = fs.readFileSync(
      path.join(root, "memory", "durable.md"),
      "utf8",
    );
    ...
    expect(
      fs.existsSync(path.join(root, "skills", "high-confidence-skill", "SKILL.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(root, "skills", "low-confidence-skill", "SKILL.md")),
    ).toBe(false);
  });
  ...
  it("respects dedupeHints to skip duplicate candidates", () => {
    ...
    expect(
      fs.existsSync(path.join(root, "skills", "unique-skill", "SKILL.md")),
    ).toBe(true);
    expect(
  ...
  it("updates existing skill when slug is in dedupeHints", () => {
    ...
    const initialState = JSON.parse(
      fs.readFileSync(path.join(root, "state.json"), "utf8"),
    ) as { skills: Record<string, { summary: string; reviewId: string }> };
    ...
    const updatedState = JSON.parse(
      fs.readFileSync(path.join(root, "state.json"), "utf8"),
    ) as { skills: Record<string, { summary: string; reviewId: string; state: string }> };
